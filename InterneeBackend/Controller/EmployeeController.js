const AppliedEmployeesModal=require('../Model/AppliedEmlpoyeesModel')
...
const ApplyEmplooyee=async(req,res)=>{
    try {
        ...
        if(!req.file){
            return res.status(400).json({FailureMessage:"Invalid File Type Only PDFs or DOCS are allowed"})
        }
       const {name,
        email,
        phone,
        ...
        console.log("Job Application Received:", {
            name, email, phone, qualifications, experience, jobPosition,
            expectedSalary,
            coverLetter, cvPath: req.file.path,
          });
        const findjobtype=await JobModal.findOne({JobName:jobPosition})
         ...
         const info=await transporter.sendMail({
            from:"'InternHub' <amiranas761@gmail.com>",
            to:email,
...
Thank you once again for your interest in joining <strong>Anas Internee.pk</strong>. We look forward to the possibility of working together and wish you the best of luck in the hiring process.</p>`
          ...
          if (info.messageId){
            const UploadEmployees=await AppliedEmployeesModal.create({name,email,phone,qualifications,experience,jobPosition,expectedSalary,coverLetter,cv:req.file.path})
            res.status(201).json({ SuccessMessage: "Application submitted successfully!", UploadEmployees });
          }
    } catch (error) {
        res.status(500).json({FailureMessage:"Internal Server Error from Applied Employees"})
        console.log(error);
